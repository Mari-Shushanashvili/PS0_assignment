...
function drawApproximateCircle(turtle, radius, numSides) {
    ...
    var sideLength = chordLength(radius, angle);
...
function findPath(turtle, points) {
    ...
    var currentHeading = turtle.getHeading();
    for (var _i = 0, points_1 = points; _i < points_1.length; _i++) {
        var target = points_1[_i];
        var dx = target.x - currentPos.x;
        var dy = target.y - currentPos.y;
        var targetAngle = (Math.atan2(dy, dx) * 180) / Math.PI;
        var turnAngle = targetAngle - currentHeading;
        if (Math.abs(turnAngle) > 0.001) {
            instructions.push("turn ".concat(turnAngle));
            currentHeading = targetAngle;
        }
        var dist = Math.sqrt(dx * dx + dy * dy);
        if (dist > 0.001) {
            instructions.push("forward ".concat(dist));
        }
        currentPos = target;
    }
    return instructions;
